...
class Player(PlayerInterface):
    ...
    def init(self, world: World) -> None:
        ...
        try:
            load_checkpoint(self.model, BACKUP_PATH)
        except:
            self.log("daco sa dojebalo pri nacitavani")
            pass
    ...
    def get_turn_train(self, world: World) -> List[Move]:
        ...
        for id, ant in world.alive_shades.items():
            ...
            self.check_ghost_memory(id, world)
            ...
            move : Point = ant.position
            if volba < 4:
                move = ant.position + move_def[volba]
                if not world.map.can_move_to(move):
                    self.memory["rewards"][id][-1] -= 10
            else:
                self.log("Skusam bfs")
                move = bfs_find_person(world, ant.position, self.people_positions)
            
            moves.append(Move(id, move))
